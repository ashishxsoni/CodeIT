...
import { closeProfilePanel, logout } from "../redux/authSlice";
import { X, UserCircle, FileDown, Settings, LogOut } from "lucide-react";

const ProfilePanel = () => {
  const dispatch = useDispatch();
  const { isProfilePanelOpen, user } = useSelector((state) => state.auth);
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        {/* Header */}
        <div className={`flex justify-between items-center p-4 border-b transition-all 
        ...
        {/* Profile Info */}
        <div className={`flex flex-col items-center py-6 space-y-2 border-b transition-all 
          ${darkMode ? "border-gray-900" : "border-gray-200"}`}>
          <motion.img
